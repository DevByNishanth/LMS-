  ...
  async function handleStudentDelete(id) {
    try {
      const res = await axios.delete(
        `${apiUrl}api/people/remove/${sectionId}/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    <section className="w-full p-6 h-full border border-[#DBDBDB] rounded-lg">
      ...
      <div className="tab-container px-4 py-2 flex items-center gap-2 bg-[#E6E9F5] rounded-full">
        <button
          ...
          className={`w-1/2 py-2 px-3 cursor-pointer rounded-full ${selectedTab == "Teachers" ? "bg-[#08384F]  text-white" : "text-black"}`}
        >
          Teachers
        </button>
        <button
          ...
          className={`w-1/2 py-2 cursor-pointer px-3 rounded-full ${selectedTab == "Students" ? "bg-[#08384F]  text-white" : "text-black"}`}
        >
          Students
        </button>
      </div>
      ...
      <header className="mt-4 flex items-center justify-between relative">
        <h1 className="font-medium text-lg">
          ...
          <span className="text-[#0B56A4]">
            ({selectedTab === "Teachers" ? faculties.length : students.length})
          </span>
        </h1>
        <div className="btn-container">
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center cursor-pointer gap-3 text-white bg-[#08384F]  px-5 py-2 rounded-lg"
          >
            <Plus className="text-white"></Plus> Add
          </button>
          <div className="dropdown-contanier absolute top-full right-0">
            {showAddModal && (
              <ClassroomAddStudentsModal
              ...
              >
                ...
                {index !== 0 && (
                  <div
                    ...
                    className="text-gray-900 text-xl font-medium bg-gray-100 w-10 h-10 flex items-center justify-center rounded-full cursor-pointer hover:bg-red-50 transition-colors"
                  >
