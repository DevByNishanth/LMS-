...
const ClassworkDetailView = ({ selectedAssignment, setIsDetailview }) => {
  ...
  const handlePostComment = async (postId) => {
    ...
    const isAssignment =
      item.itemType === "assignment" || item.key === "Assignment";
    ...
    if (isQuestion) {
      ...
      commentUrl = `${apiUrl}api/quiz/${postId}/comment`;
      payload.quizId = postId;
    } else {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        const decoded = jwtDecode(token);
        const userName =
          decoded?.name || decoded?.username || decoded?.user?.name || "You";
  ...
  return (
    <>
      ...
      {!isStudentWorkComponent ? (
        <section className="w-full max-h-[calc(100vh-190px)] overflow-y-auto  ">
          <div className="rounded-lg border border-gray-200 bg-[#F9F9F9]">
            ...
            <div className="flex justify-end px-4 mb-2">
              <button
                onClick={() => setIsStudentWorkComponent(true)}
                className="flex items-center gap-2 bg-[#08384F]  text-white text-sm px-4 cursor-pointer py-2 rounded-lg hover:bg-[#0B56A4]/90"
              >
        ...
        <StudentWorkComponent selectedAssignment={selectedAssignment} />
      )}
    </>
  );
};
