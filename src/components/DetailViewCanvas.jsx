...
const DetailViewCanvas = ({ setIsDetailCanvas, canvasData }) => {
  ...
  const timetableRef = useRef(null); // Ref for printing
  ...
  async function fetchSubjectList() {
    try {
      const res = await axios.get(
        `${apiUrl}api/admin/faculty/${canvasData._id}/subjects`,
        {
  ...
  async function fetchTimetable() {
    try {
      const res = await axios.get(
        `${apiUrl}api/admin/faculty/${canvasData._id}/timetable`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handlePrint = () => {
    const printContent = timetableRef.current.innerHTML;
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Timetable - ${canvasData.firstName}</title>
          <style>
            body { font-family: sans-serif; padding: 20px; }
            table { width: 100%; border-collapse: collapse; margin-top: 20px; }
            th, td { border: 1px solid #ddd; padding: 12px; text-align: left; font-size: 12px; }
            th { background-color: #08384F; color: white; }
            h2 { color: #08384F; }
          </style>
        </head>
        <body>
          <h2>Timetable: ${canvasData.firstName} ${canvasData.lastName}</h2>
          ${printContent}
        </body>
      </html>
    `);
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col px-6">
            <div className="flex items-center justify-between mb-5 shrink-0">
              <div className="flex bg-gray-100 p-1.5 rounded-xl">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`px-10 py-2.5 text-xs font-bold rounded-lg transition-all ${activeTab === tab ? "bg-[#08384F] text-white shadow-lg" : "text-gray-500 hover:text-gray-700 hover:bg-gray-200"}`}
                  >
                    {tab}
                  </button>
                ))}
              ...
              {/* Print Button - Only visible for Timetable tab */}
              {activeTab === "Timetable" && timetableData.length > 0 && (
                <button
                  onClick={handlePrint}
                  className="flex items-center gap-2 bg-[#08384F] text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-[#0a4a69] transition-all shadow-md"
                >
                  <Printer size={16} />
                  Print Timetable
                </button>
            ...
            <div className="flex-1 border bg-white border-gray-700 max-h-[calc(100vh-380px)] rounded overflow-auto">
              {isDataAvailable() ? (
                <div ref={activeTab === "Timetable" ? timetableRef : null}>
                      ...
                      {activeTab === "Timetable" &&
                        timetableData.map((row, i) => (
                          <tr
                            ...
                            className="hover:bg-blue-50 transition-colors"
                          >
                            <td className="p-4 text-[#08384F] font-bold bg-gray-50/80">
                              {row.slot}
                            </td>
                            {[
                              row.mon,
                              ...
                              <td
                                key={idx}
                                className={`p-4 text-xs font-bold ${day === "FREE" || day === "--" ? "text-gray-300 italic" : "text-gray-600"}`}
                              >
                                {day}
                              </td>
                            ))}
