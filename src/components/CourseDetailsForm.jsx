...
const CourseDetailsForm = ( { formData, setFormData }) => {
    ...
    const handleOutcomeChange = (index, field, value) => {
        ...
        setFormData(prev => ({
            ...prev,
            courseOutcomes: updated
        }));
    };

    // Next button
    const handleNext = () => {
        const formattedOutcome = formData.courseOutcomes.map(outcome => ({
            [outcome.unit]: outcome.statement,
            RTBL: outcome.rtbl
        }));

        const finalData = {
            ...formData,
            courseOutcomes: formattedOutcome
        };
    ...
    return (
        <>
            <form className='h-[91%] mb-2 overflow-auto'>
