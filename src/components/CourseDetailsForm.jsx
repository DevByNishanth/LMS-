...
const CourseDetailsForm = ({ formData, setFormData, onNext }) => {
    ...
    const handleOutcomeChange = (index, field, value) => {
        const updated = formData.courseOutcomes.map((outcome, i) => 
    ...
    return (
        <>
            <form className='h-[91%] mb-2 overflow-auto' onSubmit={(e) => e.preventDefault()}>
                <div className="form-content">
                    <h1 className='font-medium text-lg bg-white sticky top-0 pb-2'>Course Details</h1>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            >
                                ...
                                {courseTypeOptions.map(type => <option key={type}>{type}</option>)}
                            </select>
                        </div>
                        <div>
                        ...
                        {formData.courseObjectives.map((obj, index) => (
                            <div key={index} className="flex gap-2 mb-2">
                                ...
                                {index === formData.courseObjectives.length - 1 && (
                                    <button type="button" onClick={addObjective} className="bg-[#08384f] text-white rounded p-2">
                                        <Plus size={18} />
                                    </button>
                                )}
                            </div>
                    ...
                    <div className="mb-4">
                        ...
                        {formData.courseOutcomes.map((outcome, index) => (
                            <div key={index} className="grid grid-cols-12 gap-2 mb-2">
                                <input type="text" value={outcome.unit} disabled className="col-span-2 border border-gray-300 rounded px-2 py-2 text-sm bg-[#e6e9f5]" />
                                <input
                                    type="text"
                                    placeholder="Enter Course Statement"
                                    value={outcome.statement}
                                    onChange={(e) => handleOutcomeChange(index, "statement", e.target.value)}
                                    className="col-span-7 border border-gray-300 rounded px-3 py-2 text-sm"
                                />
                                <select
                                    value={outcome.rtbl}
                                    onChange={(e) => handleOutcomeChange(index, "rtbl", e.target.value)}
                                    className="col-span-3 border border-gray-300 rounded px-2 py-2 text-sm"
                                >
                                    <option value="">RTBL</option>
                                    {rtblOptions.map(rtbl => <option key={rtbl}>{rtbl}</option>)}
                                </select>
                            </div>
                        ))}
                    </div>
            ...
            <div className="flex justify-end">
                <button onClick={onNext} className="bg-[#08384f] text-white px-6 py-2 rounded">Next</button>
            </div>
        </>
    )
}
